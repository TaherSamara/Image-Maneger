...
export class FolderComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal,
    ...
    this.isInfoLoading$ = this.otherHTTPService.isInfoLoading$;
  ...
  // ------------ Get Folder Info ------------
  getFolderInfo(): void {
    this.otherHTTPService.GetFolder(this.folderID).pipe(first())
      .subscribe((res: any | undefined) => {
        this.folderName = res.name;
        this.folderParentID = res.parent_id;
      });
  }
  ...
  fetchAllFiles(p: number): void {
    this.page = p;
    this.otherHTTPService.List2(this.userId, 0, this.searchText, this.folderID, this.size, this.page)
      .pipe(first())
      .subscribe((res: any | undefined) => {
        this.Files = res.data;
  ...
  // ------------ Fetch Folders ------------
  fetchFolders(): void {
    this.otherHTTPService.List2(this.userId, 1, this.searchText, this.folderID, 200, this.page)
      .pipe(first())
      .subscribe((res: any | undefined) => {
  ...
  // ------------ Fetch Images ------------
  fetchImages(): void {
    this.otherHTTPService.List2(this.userId, 2, this.searchText, this.folderID, 200, this.page)
      .pipe(first())
      .subscribe((res: any | undefined) => {
  ...
  addFolder(name: string): void {
    ...
    if (this.FileForm.controls['name'].valid) {
      this.otherHTTPService.Add(name, this.folderID, this.userId).pipe(first())
        .subscribe(() => {
  ...
  addImage(): void {
    ...
    if (this.FileForm.controls['name'].valid) {
      ...
      this.otherHTTPService.Upload(description ? description : "", this.folderID, this.userId, this.files)
        .pipe(first())
        .subscribe(() => {
          this.modalService.dismissAll();
